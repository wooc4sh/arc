...
\subsection*{Klasyfikacja grup rzędu do 8.}
\begin{context}
Przechodzimy teraz do klasyfikacji grup rzędu $\le 8$ z dokładnością do $\cong$.
\end{context}

Zauważmy, że dla danej grupy $G$ i $p = \left| G \right|$, jeśli $p$ jest liczbą pierwszą, to $G\cong\mathbb{Z}_{p}$. Zatem, gdy $\left| G \right| \in \{1,2,3,5,7\} $, to $G\cong\mathbb{Z}_{\left| G \right| }$.

Ponadto wcześniej już pokazaliśmy, że jeśli $\left| G \right| = 4$ to $G\cong\mathbb{Z}_{4}$ lub $G\cong \mathbb{Z}_2\times \mathbb{Z}_2$. Pozostały przypadki 6 i 8.

\noindent \textbf{Przypadek $\left| G \right| = 6$}: Pokażemy, że $G\cong\mathbb{Z}_6$ lub $G\cong S_{3}$.
Zauważmy, że istnieje $g\in G\setminus \{e\} $ taki że rząd($g$ ) $\neq 2$
\begin{proof}
	Jeśli nie to grupa by była abelowa i to doprowadzi do sprzeczności TOD. 
\end{proof} 
Rozważmy przypadki
\begin{enumerate}[label=\arabic*.]
	\item Jeżeli $\text{rząd}\left( g \right) = 6$ to $\langle g \rangle \cong Z_{6}$ 
	\item Jeżeli $\text{rząd}\left( g \right) \neq 6$. Wtedy możliwe rzędy to 2 i 3. Załóżmy, że $\text{rząd}\left( g \right) = 3$.
		Niech $N= \langle g \rangle \le G$ wtedy $[G:N] = \frac{6}{3}=2$ zatem $N\unlhd G.$ 

		Rozważmy element $h \in G \setminus N$ i niech $H = \langle h \rangle$.
		\begin{itemize}
    			\item Przypadek $\text{ord}(h) = 6$ odrzucamy z założenia (wtedy $G \cong \mathbb{Z}_6$).
    			\item Jeśli $\text{ord}(h) = 3$, to $H \neq N$, ponieważ $h \notin N$. Skoro $|H| = |N| = 3$ są liczbami pierwszymi, ich przecięcie musi być trywialne: $H \cap N = \{e\}$. Wówczas, zgodnie ze wzorem na rząd iloczynu podgrup, otrzymujemy:
   			 	$$|NH| = \frac{|N| \cdot |H|}{|N \cap H|} = \frac{3 \cdot 3}{1} = 9.$$
    				Ponieważ $|NH| \le |G| = 6$, otrzymujemy sprzeczność. 
    			\item Zatem jedyną możliwością pozostaje $\text{ord}(h) = 2$. Wtedy $|H| = 2$, skąd wynika $H \cap N = \{e\}$ (rzędy podgrup są względnie pierwsze). Ponieważ $N \unlhd G$, $H \le G$, $H \cap N = \{e\}$ oraz $|NH| = \frac{3 \cdot 2}{1} = 6$, grupa $G$ jest wewnętrznym produktem półprostym:
  				  $$G = N \rtimes H \cong \mathbb{Z}_3 \rtimes \mathbb{Z}_2 \cong S_3.$$
		\end{itemize}
...
\toprule 
Rząd & Grupy abelowe & Grupy nieabelowe \\
\midrule
1 & $\mathbb{Z}_1$ & -- \\
\addlinespace
2 & $\mathbb{Z}_2$ & -- \\
...
7 & $\mathbb{Z}_7$ & -- \\
\addlinespace
8 & $\mathbb{Z}_8$, $\mathbb{Z}_4 \times \mathbb{Z}_2$, \quad $\mathbb{Z}_2 \times \mathbb{Z}_2 \times \mathbb{Z}_2$ & $D_4$, $Q_8$ \\
\bottomrule
\end{tabularx}
\end{table}

\subsection*{Równoważność działania i pewnego homomorfizmu $\varphi$ Tymczasowo tutaj}
