...
\section{Pierścień szeregów formalnych i wielomiany.}
...
\begin{definition}[R-moduł.]
	Niech $R$ będzie pierścieniem przemiennym z $\mathbf{1}$. $M$ jest \textbf{R-modułem} gdy
	\begin{enumerate}[label=\arabic*)]
		\item $\left(M, +  \right) $ jest grupą przemienną.
		\item Mamy funkcję \begin{align*}
			f: R\times M &\longrightarrow M \\
			\left( r,m \right)  &\longmapsto f(\left( r,m \right) ) = r\cdot m		
		.\end{align*}
		(jest to mnożenie przez skalary z $R$), która spełnia $\left(\forall r,r' \in R\right)\left(\forall m,m'\in M \right) $:
		\begin{enumerate}[label=\roman*)]
			\item $r\left( m+m' \right) = r\cdot m + r\cdot m'$,
			\item $\left( rr' \right) \cdot m = r\cdot \left( r'\cdot m \right) $,
			\item $\left( r+r' \right) \cdot m = r\cdot m + r'\cdot m$,
			\item $\mathbf{1}\cdot m = m$.
		\end{enumerate}
	\end{enumerate}
\end{definition} 
...
\[
\varphi :R \to \text{End}\left( \left( M,+ \right)  \right), \text{ dany wzorem } \varphi \left( r \right) \left( m \right) \text{, taki że } \varphi \left( \mathbf{1} \right) = \id_{M}   
.\] 

Taki homomorfizm jest równoważny pojęciu modułu (podobnie jak dla działań grupy).

\begin{remark}[Przestrzeń liniowa.]
	Jeśli $R=K$ jest ciałem, to $K$-moduły nazywamy przestrzeniami liniowymi. 

	Tak samo jak dla przestrzeni liniowej definiuje się liniową $R$ niezależność w dowolnym $R$-module $M$ i pojęcie bazy. Nie każdy $R$-moduł ma bazę, jeśli $R$-moduł $M$ ma bazę, to $M$ nazywamy $R$-modułem wolnym. Na przykład $R$ jest $R$-modułem wolnym o bazie $\{\mathbf{1}\} $. 

	Każdy $R$-moduł wolny jest izomorficzny z $R^{\oplus \kappa}$, $\kappa$ jest mocą bazy. 

	\noindent Jeśli $R\underset{\scriptscriptstyle\text{podp}}{\subseteq} S$, to $S$ staje się $R$-modułem.

	Na przykład $\mathbb{R} ,\mathbb{Q} $ to przestrzenie liniowe nad $\mathbb{Q} $ oraz $\mathbb{R} \cong \mathbb{Q} ^{\oplus \mathfrak{c}}$ i $\mathbb{C} \cong  \mathbb{Q} ^{\oplus \mathfrak{c}}$ jako przestrzeń liniowa czyli też jako grupa. 

	Każda grupa przemienna $\left( A,+ \right) $ jest $\mathbb{Z} $-modułem, bo
	\[
	\left(\forall n\in \mathbb{Z}  \right) \left(\forall a \in A \right)\left( n\cdot a := \underbrace{a+a+\ldots+a}_{n\text{ krotne dodawanie}} \text{ spełnia definicję modułu}\right)  
	.\] 

	Wolne $\mathbb{Z} $-moduły to dokładnie wolne grupy abelowe.
\end{remark} 
